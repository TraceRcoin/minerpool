...
exports.addressToScript = function(addr){
    ...
    // bech32 (native segwit) support for tfx/ttfx/rtfx HRPs
    var seg = exports.segwitAddressToScript(addr);
    if (seg) return seg;
...
// ---- bech32 / bech32m segwit decoding (BIP173/BIP350) ----
...
function bech32Decode(str){
    var lower=str.toLowerCase(), upper=str.toUpperCase();
    ...
    for (var i=pos+1;i<str.length;++i){
        var d=BECH32_CHARSET.indexOf(str[i]);
        if (d===-1) return null;
        data.push(d);
    }
    var poly=bech32Polymod(bech32HrpExpand(hrp).concat(data));
    // accept both bech32 (1) and bech32m (0x2bc830a3); v0 uses bech32
    if (poly!==1 && poly!==0x2bc830a3) return null;
...
exports.segwitAddressToScript = function(addr){
    var dec = bech32Decode(addr);
    if (!dec || dec.data.length<1) return null;
    var version = dec.data[0];
    if (version>16) return null;
    var program = bech32ConvertBits(dec.data.slice(1), 5, 8, false);
    if (!program || program.length<2 || program.length>40) return null;
    if (version===0 && program.length!==20 && program.length!==32) return null;
    var op = version===0 ? 0x00 : (0x50 + version);
    return Buffer.concat([ new Buffer([op, program.length]), new Buffer(program) ]);
};
